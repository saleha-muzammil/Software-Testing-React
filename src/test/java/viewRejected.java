...
public class viewRejected {
    ...
    @Test
    public void testRejectedStudent() {
...
wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("loading")));
    ...
    homeButton.click();
          ...
          List<WebElement> rejectedRequests = new WebDriverWait(driver, Duration.ofSeconds(10))
    .until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("button.buttons-hover")));

WebElement newReq = rejectedRequests.get(3);
...
List<WebElement> noRequestsMessage = driver.findElements(By.xpath("//h1[contains(text(), 'No Request Found')]"));
    ...
    if (!noRequestsMessage.isEmpty()) {
        ...
        System.out.println("No rejected requests found. Test successful.");
        return;
    }
     ...
     @Test
    public void testRejectedTeacher() {
        
        driver.get("http://localhost:3000");


 WebElement academicStaffButton = new WebDriverWait(driver, Duration.ofSeconds(10))
        .until(ExpectedConditions.elementToBeClickable(By.xpath("//button[text()='Sign in as Academic Staff']")));

    // Use JavaScript to click the Academic Staff sign-in button
    ((JavascriptExecutor) driver).executeScript("arguments[0].click();", academicStaffButton);
    ...
    emailInput.sendKeys("teacher@gmail.com");
    passwordInput.sendKeys("123");
    loginButton.click();
...
WebElement newReq = rejectedRequests.get(2);
((JavascriptExecutor) driver).executeScript("arguments[0].click();", newReq);
